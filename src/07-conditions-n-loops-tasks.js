...
function getSumBetweenNumbers(n1, n2) {
  return Array(n2 - n1 + 1).fill(n1).map((elem, index) => elem + index)
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const specSymbols = ['[', ']', '(', ')'];
  let interval = `[${[a, b].sort((x, y) => x - y).join(', ')}]`;
  if (!isStartIncluded) interval = interval.replace(specSymbols[0], specSymbols[2]);
  if (!isEndIncluded) interval = interval.replace(specSymbols[1], specSymbols[3]);
  return interval;
}
...
function isCreditCardNumber(ccn) {
  let sum = 0;
  const strCNN = String(ccn);
  let even = false;
  for (let i = strCNN.length - 1; i >= 0; i -= 1) {
    let digit = +strCNN[i];
    if (even) {
      digit *= 2;
      if (digit > 9) {
...
function getDigitalRoot(num) {
  return num.toString().split('').map((elem) => +elem).reduce((acc, elem) => acc + elem, 0)
    .toString()
    .split('')
    .map((elem) => +elem)
    .reduce((acc, elem) => acc + elem, 0);
